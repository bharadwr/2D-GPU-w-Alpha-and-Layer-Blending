...
AHB_MASTERINTERFACE_FILE	:= AHB_MasterInterface.sv
BRESENHAM_FILE			:= bresenham.sv
APB_SLAVEINTERFACE_FILE		:= APB_SlaveInterface.sv
CONTROLLER_FILE			:= controller.sv
DECODE_PRO_FILE			:= decode_pro.sv
FLEX_COUNTER_FILE		:= flex_counter.sv
CLEAR_FILE			:= clear.sv
MEMORYMANAGER_FILE		:= memoryManager.sv
RENDER_MODULE_FILE		:= render_module.sv
RENDER_RCU_FILE			:= render_rcu.sv
...
# The contents of these variables must either be plain text (no functions or variable references)
# or references to the Phase 1 submodule code file variables
COMPONENT_FILES	:= $(AHB_MASTERINTERFACE_FILE) $(APB_SLAVEINTERFACE_FILE) $(BRESENHAM_FILE) $(CLEAR_FILE) $(CONTROLLER_FILE) $(DECODE_PRO_FILE) $(FLEX_COUNTER_FILE) $(MEMORYMANAGER_FILE) $(RENDER_MODULE_FILE) $(RENDER_RCU_FILE)
